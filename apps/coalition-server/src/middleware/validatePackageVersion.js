...
async function validateVersion(req, res, next) {
  ...
  if (version !== req.packageVersion) {
    // deepcode ignore UsageOfUndefinedReturnValue: We want our server to perform a redirect, but we also want to break out of the function early.
...
const asyncValidateVersion = asyncHandler(validateVersion);

export { asyncValidateVersion as validateVersion };
